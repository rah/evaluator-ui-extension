...
library(shiny)
library(scales)
# library(dpylr) is a part of tidyverse
library(viridis)
...
flexdashboard::renderValueBox({
    if (values$modelLoaded) {
        ...
        flexdashboard::valueBox(
                           dollar(dat)
                           caption = "95% Value at Risk", icon = "fa-pencil",
                           color = case_when(
                               dat >= risk_tolerances[
                                          risk_tolerances$level == "high",]$amount ~ "High",
                               dat >= risk_tolerances[
                                          risk_tolerances$level == "medium",]$amount ~ "Medium",
                               TRUE ~ "success")
        }
    }
})
